...
class Apostador(models.Model):
    ...
    usuario = models.OneToOneField(
        ...
        primary_key=True,
        # renomei a chave para evitar erros na hora da migracao
        related_name='apostadorusuario',
    )
...
class Selecao(models.Model):
    ...
    class Meta:
        # Nome que de fato sera exibido no site para a tabela no plural
        verbose_name_plural = "Seleções"
        # odenacao que sera realizada, pode ser mais de um campo (- ordem decrescente)
        ordering = ["nome"]
...
class Bolao(models.Model):
    ...
    def adicionar_aposta(self, aposta):
        ...
        apostador = Apostador.objects.filter(
            usuario_id=aposta.apostador_id).first()

        # verificar aqui se o apostador já possui alguma aposta e não lá na view
        ...
        if apostador.credito >= aposta.valor:  # so permite que a aposta seja efetivada caso o credito do apostador seja maior ou igual ao valor da aposta
            ...
            # reduz o valor do credito do apostador
            apostador.apostar_valor(aposta.valor)
            # colocar estre três save em uma transação: tudo ou nada
            apostador.save()  # salva o apostador
            self.save()  # salva o bolao
            aposta.save()  # salva a aposta
        else:
            raise ValueError("Seu crédito atual é de >> R$ " + str(
                aposta.apostador.credito) + " << insuficiente para realizar uma aposta!")
    ...
    def remover_aposta(self, aposta):
        ...
        # devolve o valor da  aposta no credito do apostador
        aposta.apostador.adicionar_credito(aposta.valor_aposta)
    ...
    def set_premiacao(self, apostadores, vencedores):
        ...
        if vencedores:  # se a lista nao estiver vazia divide o valor disputado pela quantidade de vencedores seta a premiacao
            ...
            for ven in vencedores:  # distribui a premiacao para os vencedores
                apostador = Apostador.objects.filter(
                    usuario_id=ven.apostador_id).first()
                apostador.premiacao_ganha += Decimal(self.premiacao)
                apostador.qtd_vitorias += 1
                apostador.save()
                # converte a premiacao em credito
                apostador.adicionar_credito(self.premiacao)
    ...
    def verificar_vencedores(self, apostas, vencedores):
        ...
        # ao final calcula a premiacao
        self.set_premiacao(apostadores, vencedores)


class ApostadorVencedorBolao(models.Model):
    class Meta:
        # cria a relacao semelhante a de chave composta no Django
...
# gera o formulario de cadastro de usuarios de forma automatica ESTUDAR PARA APLICAR
