...
def login(request):
    ...
    if not usuario or not senha:
        messages.error(
            request, "Informe  usuário e senha para realizar o login no sistema!")
        return render(request, 'bolao_app/login.html')
...
def dashboard(request):
    ...
    apostador_credito = Apostador.objects.filter(
    ...
    # colocar esta logica dentro do método adicionar_aposta no medel Bolao
    if Aposta.objects.filter(apostador_id=apostador_id).filter(bolao_id=boloao_id).exists():
        messages.error(
            request, 'Você já possui uma aposta para este bolão! É permitido apostar apenas uma vez por bolão!')
        return render(request,
                      'bolao_app/dashboard.html',
                      {'boloes': boloes,
                       'apostador_credito': apostador_credito,
...
@login_required(redirect_field_name='login')
def ranking(request):
    # usuarios = User.objects.order_by('-apostadorusuario__premiacao_ganha').filter()
    usuarios = User.objects.order_by(
        '-apostadorusuario__premiacao_ganha').exclude(apostadorusuario__premiacao_ganha__isnull=True)
    return render(request,
...
def admin_register_result(request):
    ...
    partidas = Partida.objects.filter(
        gols_desafiante=None).filter(gols_visitante=None).all()
    ...
    partidas = Partida.objects.filter(gols_desafiante=None).filter(
        gols_visitante=None).filter(finalizada=True).all()
